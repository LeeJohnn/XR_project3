...
namespace VRTK
{
    ...
    /// </example>
    [ExecuteInEditMode]
    public class VRTK_SnapDropZone : MonoBehaviourPunCallbacks
    {
...
#pragma warning restore 618
            ...
            if (willSnap && !isSnapped && ValidSnapObject(interactableObjectCheck, false))
            {
                ...
                if (!interactableObjectCheck.IsInSnapDropZone())
                {
                    ...
                    if (cloneNewOnUnsnap)
                    {
                        ...
                        //Debug.Log(cnt);
                        if(cnt == 100)
                            this.GetComponent<VRTK_SnapDropZone>().ForceUnsnap();
                    }
        ...
        protected virtual void CreatePermanentClone()
        {
            if (isGenerate)
            {
                ...
                Debug.Log("hi");
                objectToClone = PhotonNetwork.Instantiate(prefab_name, currentSnappedObject.gameObject.transform.position, currentSnappedObject.gameObject.transform.rotation);
